...
import com.alibaba.link.ModuleLoader;
...
public class ModuleLoaderImpl implements ModuleLoader, ApplicationContextAware {
     ...
     * Spring bean文件所在目录
     */
    private static final String SPRING_XML_PATTERN3 = "classpath*:%s/link-*.xml";
    ...
    private ClassPathXmlApplicationContext loadModuleApplication(ModuleConfig moduleConfig, List<String> tempFileJarURLs) {
        ...
        try {
            ...
            moduleApplicationContext.setConfigLocations(findSpringConfigs(moduleConfig.getPackageDir(), tempFileJarURLs, moduleClassLoader,
     ...
     * @return
     */
    private String[] findSpringConfigs(String packageDir, List<String> tempFileJarURLs, ClassLoader moduleClassLoader, List<String> exclusionConfigeNameList) {
        try {
            PathMatchingResourcePatternResolver pmr = new PathMatchingResourcePatternResolver(moduleClassLoader);
//            Resource[] springXmlResource = pmr.getResources(SPRING_XML_PATTERN);
//            Resource[] springXml2Resource = pmr.getResources(SPRING_XML_PATTERN2);
            Resource[] packageXmlResource = pmr.getResources(String.format(SPRING_XML_PATTERN3, packageDir));

            Resource[] springXmlPattern = ImmutableSet.builder()
                    .add(packageXmlResource)
                    .build().toArray(new Resource[]{});
            checkNotNull(springXmlPattern, "resources is null");
            checkArgument(springXmlPattern.length > 0, "resources length is 0");
            // 因为ClassLoader是树形结构，这里会找到ModuleClassLoader以及其父类中所有符合规范的spring配置文件，所以这里需要过滤，只需要Module Jar中的
            return filterURLsIncludedResources(tempFileJarURLs, springXmlPattern, exclusionConfigeNameList);
        } catch (IOException e) {
    ...
    private String[] filterURLsIncludedResources(List<String> tempFileJarURLs, Resource[] resources, List<String> exclusionConfigeNameList)
        ...
        for (Resource resource : resources) {
            ...
            for (String jarUrl : tempFileJarURLs) {
                ...
                if (configLocation.contains(jarUrl)) {
